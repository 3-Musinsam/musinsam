...
public class OrderControllerApiV1Tests {
  ...
  void createOrderSuccess() throws Exception {
    // Given
    OrderItem orderItem1 = OrderItem.builder().id(UUID.randomUUID()).productName("ProductName1")
        .price(new BigDecimal("10000")).quantity(5).build();

    OrderItem orderItem2 = OrderItem.builder().id(UUID.randomUUID()).productName("ProductName2")
        .price(new BigDecimal("20000")).quantity(10).build();

    ShippingInfo shippingInfo = ShippingInfo.builder().address("Shipping Address")
        .addressDetail("Shipping Address Detail").receiverName("John Doe")
        .receiverPhone("010-1234-5678").zipCode("12345").build();

    ReqOrderPostDtoApiV1 requestDto = ReqOrderPostDtoApiV1.builder().order(
        Order.builder().orderItems(List.of(orderItem1, orderItem2))
            .totalAmount(new BigDecimal("250000")).discountAmount(new BigDecimal("25000"))
            .finalAmount(new BigDecimal("225000")).shippingInfo(shippingInfo)
            .request("Request Example").couponId(UUID.randomUUID()).build()).build();

    ResOrderPostDtoApiV1 responseDto = ResOrderPostDtoApiV1.builder().order(
        ResOrderPostDtoApiV1.Order.builder().id(UUID.randomUUID()).userId(1L).orderStatus("PENDING")
            .orderItems(List.of(
                ResOrderPostDtoApiV1.Order.OrderItem.builder().productId(UUID.randomUUID())
                    .productName("ProductName1").price(new BigDecimal("10000")).quantity(5).build(),
                ResOrderPostDtoApiV1.Order.OrderItem.builder().productId(UUID.randomUUID())
                    .productName("ProductName2").price(new BigDecimal("20000")).quantity(10)
                    .build())).totalAmount(new BigDecimal("250000"))
            .discountAmount(new BigDecimal("25000")).finalAmount(new BigDecimal("225000"))
            .request("Request Example").createdAt(ZonedDateTime.now()).build()).build();

    when(orderService.createOrder(any(ReqOrderPostDtoApiV1.class), any())).thenReturn(responseDto);

    // When & Then
    mockMvc.perform(post("/v1/orders").header("X-USER-ID", 1L).header("X-USER-ROLE", "ROLE_USER")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(requestDto))).andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value(0)).andExpect(jsonPath("$.message").exists())
        .andDo(print()).andDo(MockMvcRestDocumentationWrapper.document("order-create-success",
            preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), resource(
                ResourceSnippetParameters.builder().tag("Order V1").summary("주문 생성 성공")
                    ...
                    .requestFields(fieldWithPath("order").description("주문 정보"),
                    ...
                    .responseFields(fieldWithPath("code").description("응답 코드"),
                        ...
                        fieldWithPath("data.order.createdAt").description("주문 생성 일시")).build())));
  }
  ...
  void getOrderByIdSuccess() throws Exception {
    ...
    mockMvc.perform(RestDocumentationRequestBuilders.get("/v1/orders/{orderId}", orderId)
            .header("X-USER-ID", 1L).header("X-USER-ROLE", "ROLE_USER")
            .param("orderId", String.valueOf(orderId)).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk()).andExpect(jsonPath("$.code").value(0))
        .andExpect(jsonPath("$.message").exists()).andDo(
            MockMvcRestDocumentationWrapper.document("order-getById-success",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), resource(
                    ResourceSnippetParameters.builder().tag("Order V1").summary("주문 상세 조회 성공")
                        .requestHeaders(headerWithName("X-USER-ID").description("사용자 ID"),
                            headerWithName("X-USER-ROLE").description("사용자 권한")).build())));
  }
  ...
  void getOrderSuccess() throws Exception {
    ...
    // When & Then
    mockMvc.perform(RestDocumentationRequestBuilders.get("/v1/orders").header("X-USER-ID", 1L)
            .header("X-USER-ROLE", "ROLE_USER").param("q", query).param("orderId", orderId.toString())
            .param("page", "0").param("size", "10").param("sort", "createdAt,desc")
            .contentType(MediaType.APPLICATION_JSON)).andDo(print()).andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value(0)).andExpect(jsonPath("$.message").exists()).andDo(
            MockMvcRestDocumentationWrapper.document("order-get-success",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), resource(
                    ResourceSnippetParameters.builder().tag("Order V1").summary("주문 목록 조회 성공")
                        .requestHeaders(headerWithName("X-USER-ID").description("사용자 ID"),
                            headerWithName("X-USER-ROLE").description("사용자 권한"))
                        .queryParameters(parameterWithName("q").description("검색어").optional(),
  ...
  void updateOrderSuccess() throws Exception {
    ...
    ReqOrderPatchDtoApiV1 requestDto = ReqOrderPatchDtoApiV1.builder()
        .order(ReqOrderPatchDtoApiV1.Order.builder().request("Sample Request Message").build())
        .build();

    ResOrderPatchDtoApiV1 responseDto = ResOrderPatchDtoApiV1.builder().order(
        ResOrderPatchDtoApiV1.Order.builder().id(orderId).request("Sample Request Message")
            .updatedAt(ZonedDateTime.now()).build()).build();
    ...
    mockMvc.perform(RestDocumentationRequestBuilders.patch("/v1/orders/{orderId}", orderId)
        ...
        .andExpect(jsonPath("$.code").value(0)).andExpect(jsonPath("$.message").exists()).andDo(
            MockMvcRestDocumentationWrapper.document("order-patch-success",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), resource(
                    ResourceSnippetParameters.builder().tag("Order V1").summary("주문 수정 성공")
                        .requestHeaders(headerWithName("X-USER-ID").description("사용자 ID"),
                            headerWithName("X-USER-ROLE").description("사용자 권한")).build())));
  }
  ...
  void cancelOrderSuccess() throws Exception {
    ...
    ReqOrderPostCancelDtoApiV1 requestDto = ReqOrderPostCancelDtoApiV1.builder()
        .cancelReason("Sample Cancel Reason").build();

    ResOrderPostCancelDtoApiV1 responseDto = ResOrderPostCancelDtoApiV1.builder().order(
        ResOrderPostCancelDtoApiV1.Order.builder().id(orderId).orderStatus("CANCELLED")
            .cancelReason("Sample Cancel Reason").canceledAt(ZonedDateTime.now()).build()).build();
    ...
    mockMvc.perform(RestDocumentationRequestBuilders.post("/v1/orders/{orderId}/cancel", orderId)
        ...
        .andExpect(jsonPath("$.message").exists()).andDo(
            MockMvcRestDocumentationWrapper.document("order-cancel-success",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), resource(
                    ResourceSnippetParameters.builder().tag("Order V1").summary("주문 취소 성공")
                        .requestHeaders(headerWithName("X-USER-ID").description("사용자 ID"),
                            headerWithName("X-USER-ROLE").description("사용자 권한")).build())));
  }
}
